...
func _on_LeftLimit_body_entered(body: Ball):
	scoreP2 += 1
	restartTimer.start()

func _on_RightLimit_body_entered(body: Ball):
	scoreP1 += 1
	restartTimer.start()
...
func _on_LeftPaddleAIArea_body_exited(body: Ball):
	p1Paddle.ball = null


func _on_RightPaddleAIArea_body_exited(body):
	p2Paddle.ball = null
